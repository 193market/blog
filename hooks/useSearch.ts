...
export const useSearch = () => {
  ...
  const search = useCallback(async (keyword: string, feature: Feature, source: SearchSource, options?: { naverClientId?: string, naverClientSecret?: string, isServerConfigured?: boolean }) => {
    if (!keyword.trim()) {
    ...
    try {
      ...
      if (feature === 'keywords') {
        ...
        if (!options?.isServerConfigured && (!options?.naverClientId || !options?.naverClientSecret)) {
