...
const callNaverApi = async (endpoint: string, params: Record<string, string>, clientId: string, clientSecret: string) => {
    ...
    // Use our backend proxy instead of public CORS proxies
    const typeMatch = endpoint.match(/\/v1\/search\/(.+)\.json/);
    const type = typeMatch ? typeMatch[1] : 'blog';
    const targetUrl = `/api/naver/search/${type}?${queryString}`;
    ...
    try {
        ...
        if (!response.ok) {
            const errorMessage = responseData.errorMessage || responseData.error || `HTTP status ${response.status}`;
...
const fetchSingleTermKeywords = async (term: string, source: SearchSource): Promise<string[]> => {
    const config = (source: SearchSource) => {
        if (source === 'naver') {
            return {
                url: `/api/autocomplete/naver?q=${encodeURIComponent(term)}`,
                parser: (data: any) => {
        ...
        // Default to Google
        return {
            url: `/api/autocomplete/google?q=${encodeURIComponent(term)}`,
            parser: (data: any) => {
